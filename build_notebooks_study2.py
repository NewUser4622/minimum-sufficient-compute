...
df0 = pd.read_parquet(runs_dir / runs[0] / 'per_sample' / 'test.parquet')
missing = [c for c in SCORES + ['sample_idx', 'label'] if c not in df0.columns]
exits = sorted(c for c in df0.columns if c.startswith('pred_d'))
print(f'\\nper-exit columns: {{exits}}')
print(f'missing: {{missing if missing else "none"}}')
assert not missing, f'inventory is wrong -- absent: {{missing}}'
print('\\nP-1 OK: the artifacts match 03_INVENTORY.md')
"""),
        md("""
---
...
def score_matrix(run_id):
    ...
    return d.sort_values('sample_idx')[SCORES].to_numpy(dtype=float)

X = score_matrix(runs[0])
ok = ~np.isnan(X).any(axis=1)
C, _ = spearmanr(X[ok])
cm = pd.DataFrame(np.abs(C), index=SCORES, columns=SCORES)

print(f'|Spearman| between scores, run {runs[0]}  (n={int(ok.sum()):,})')
print(cm.round(2).to_string())

thr = 0.90
pairs = [(a, b, cm.loc[a, b]) for i, a in enumerate(SCORES)
         for b in SCORES[i + 1:] if cm.loc[a, b] >= thr]
print()
if pairs:
...
rows = []
for arch, grp in base.groupby('arch'):
    ids = sorted(grp['run_id'])
    if len(ids) < 2:
        continue
    frames = {r: pd.read_parquet(runs_dir / r / 'per_sample' / 'test.parquet')
    ...
    for a, b in itertools.combinations(ids, 2):
        ...
        for s in SCORES:
            ...
            if m.sum() < 100 or np.std(va[m]) == 0 or np.std(vb[m]) == 0:
                continue
            rho, _ = spearmanr(va[m], vb[m])
            rows.append({'arch': arch, 'score': s, 'pair': f'{a[-2:]}|{b[-2:]}',
                         'rho_seed': float(rho), 'n': int(m.sum())})

pairs_df = pd.DataFrame(rows)
grid = (pairs_df.groupby(['arch', 'score'])['rho_seed']
...
wide = grid.pivot(index='arch', columns='score', values='rho_seed')
print('rho_seed  (rows = architecture, cols = score)')
...
"""),
        code("""
vals = grid['rho_seed'].dropna()
rng = float(vals.max() - vals.min())
by_score = grid.groupby('score')['rho_seed'].agg(['min', 'max', 'mean'])
...
budgets = {}
def rho_for(arch):
    if arch not in budgets:
        budgets[arch] = sess.budgets(arch)['axes']['depth']['rho']
    return list(budgets[arch])
...
for arch, grp in base.groupby('arch'):
    ...
    for i, j in itertools.permutations(ids, 2):
        ...
        for s in SCORES:
            ...
            rows.append({'arch': arch, 'score': s, 'model_seed': i[-2:],
...
j = (bias.groupby(['arch', 'score'])['bias'].median().reset_index()
